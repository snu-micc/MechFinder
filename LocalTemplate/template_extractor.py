...
def get_frag_around_tetrahedral_center(mol, idx):
    ...
    return Chem.MolFragmentToSmiles(mol, ids_to_include, isomericSmiles=USE_STEREOCHEMISTRY,
...
def get_strict_smarts_for_atom(atom, special_group = False):
    '''
    ...
    if USE_ATOM_SYMBOL or special_group:
        ...
        symbol = '[%s:%s]' % (general_symbol, atom.GetAtomMapNum())
        if atom.GetIsAromatic():
            symbol = symbol.lower()
...
def add_atom_num(reaction):
    ...
    for r in rs.split('.'):
        ...
        if r in ps: # reagent
            [atom.SetAtomMapNum(0) for atom in rmol.GetAtoms()]
        if sum([atom.GetAtomMapNum() > 0 for atom in rmol.GetAtoms()]) > 0:
...
#     add_atom_num(reaction)
    ...
    return reaction['reactants']+'>>'+reaction['products'], canonical_template, replacement_dict